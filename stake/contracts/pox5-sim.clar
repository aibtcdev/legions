...
(define-data-var bond-admin principal 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM) ;; SIMNET: we are the admin
...
(define-constant SIM-SKIP-HEADER true) ;; SIMNET ONLY
...
(define-private (validate-l1-lockup
    ...
    (let (
        ...
        (asserts! (or SIM-SKIP-HEADER (verify-block-header (get header lockup) (get height lockup)))
...
(define-read-only (verify-block-header
    ...
    (match (get-burn-block-info? header-hash expected-block-height)
        bhh (or SIM-SKIP-HEADER (is-eq bhh (reverse-buff32 (sha256 (sha256 headerbuff)))))
        false
    )
)
