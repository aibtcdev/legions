...
(define-public (stake
  ...
  (begin
    ...
    (try! (contract-call? .legion-treasury-gemma4 deposit ft amount))
...
(define-public (unstake
  ...
  (let (
    ...
    (try! (contract-call? .legion-treasury-gemma4 execute-transfer ft staker refund))
    (print {
      event: "unstake",
      staker: staker,
      amount: amount,
...
(define-public (conclude-proposal
  ...
  (let (
    ...
    (if votePassed
      (try! (contract-call? .legion-treasury-gemma4 execute-transfer ft (get recipient prop)
        (get amount prop)
      ))
      true
    )
