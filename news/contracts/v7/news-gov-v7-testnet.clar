...
(define-read-only (quote-payout)
  (/ (* (contract-call? .news-treasury-v7-testnet get-balance) PAYOUT_BPS) u10000)
)

;; Weight a contribution would mint now
(define-read-only (quote-weight (amount uint))
  (let (
      (bal (contract-call? .news-treasury-v7-testnet get-weighted-balance))
...
(define-read-only (propose-status (who principal))
  (let (
      (pool (contract-call? .news-treasury-v7-testnet get-balance))
      (weight (get-weight who))
...
(define-public (contribute (amount uint))
  (let (
      (balBefore (contract-call? .news-treasury-v7-testnet get-weighted-balance))
      (total (var-get TotalWeight))
    ...
    (try! (contract-call? .news-treasury-v7-testnet contribute-in amount))
...
(define-public (conclude (proposalId uint))
  (let (
      ...
      (poolShort (> payout (contract-call? .news-treasury-v7-testnet get-balance)))
    )
      ...
      (if (not thresholdMet)
        ...
        (if poolShort
          ...
          (begin
            ...
            (unwrap!
              (contract-call? .news-treasury-v7-testnet execute-payout
